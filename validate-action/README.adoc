...
[arabic]
. *SPDX header* — Verifies `+SPDX-License-Identifier+` is present in the
first 10 lines
. *Identity fields* — Requires `+agent-id+`, `+name+`, or `+project+`
field (relaxed for AI-MANIFEST files)
. *Version field* — Checks for `+version+` or `+schema_version+`
. *Attestation blocks* — If an `+[attestation]+` section exists,
validates it contains `+proof+`, `+signature+`, or `+hash+` fields
. *Section syntax* — Warns on malformed `+[section]+` headings with
unclosed brackets
