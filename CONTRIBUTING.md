...
Thank you for your interest in contributing! This repository is a **coordination hub** and a satellite of `hyperpolymath/standards`: it does not own the A2ML specification or governance — those are pinned from `standards` — but it owns the conformance fixtures, the membership manifest, and the cross-repo drift CI. Human-readable documentation lives in the root; machine-readable metadata lives under `.machine_readable/`.

## How to Contribute

We welcome contributions in many forms:

- **Conformance fixtures:** Add or refine cases under `conformance/valid/` and `conformance/invalid/` (keep `conformance/manifest.a2ml` in sync).
...
4. **Validate conformance:** Run the A2ML validator (`hyperpolymath/a2ml-validate-action`) over `conformance/valid` (expect 0 errors) and over `conformance/invalid` with strict mode (every case must error).
