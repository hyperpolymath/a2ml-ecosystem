...
import a2ml_gleam/types.{
  Attestation, AttestationBlock, Automated, Directive, DirectiveBlock, Heading,
  Paragraph, Reviewed, Text, Unverified, Verified,
}
import gleeunit
...
pub fn parse_empty_input_test() {
  ...
  let assert True = result == Error(parser.EmptyInput)
}

pub fn parse_title_only_test() {
  let assert Ok(doc) = parser.parse("# Hello A2ML")
  let assert True = doc.title == Ok("Hello A2ML")
}
...
pub fn parse_attestation_test() {
  ...
  let assert True = a.identity == "Alice"
  let assert True = a.role == "reviewer"
  let assert True = a.trust_level == Reviewed
}
...
pub fn parse_full_document_test() {
  ...
  let assert Ok(doc) = parser.parse(input)
  let assert True = doc.title == Ok("Test Doc")
...
pub fn render_trust_level_test() {
  let assert True = renderer.render_trust_level(Unverified) == "unverified"
  let assert True = renderer.render_trust_level(Automated) == "automated"
  let assert True = renderer.render_trust_level(Reviewed) == "reviewed"
  let assert True = renderer.render_trust_level(Verified) == "verified"
}
...
pub fn render_roundtrip_test() {
  ...
  let assert True = doc.title == doc2.title
}
...
pub fn extract_manifest_test() {
  ...
  let assert True = manifest.version == Ok("3.0")
  let assert True = manifest.title == Ok("Manifest Test")
}
