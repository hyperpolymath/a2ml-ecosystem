...
sed -i 's/A2ML Ecosystem Coordination Hub/my-project/g' $(grep -rl 'A2ML Ecosystem Coordination Hub' .)
sed -i 's/A2ML_ECOSYSTEM/MY_PROJECT/g' $(grep -rl 'A2ML_ECOSYSTEM' .)
sed -i 's/{{project}}/my_project/g' $(grep -rl '{{project}}' .)
sed -i 's/a2ml-ecosystem/my-project/g' $(grep -rl 'a2ml-ecosystem' .)
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `A2ML Ecosystem Coordination Hub` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `{{PROJECT_DESCRIPTION}}` | One-line description | `A tool for X` | flake.nix |
| `A2ML_ECOSYSTEM` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `{{project}}` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `a2ml-ecosystem` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
