...
* Idris2 ABI definitions (Types.idr, Layout.idr, Foreign.idr) — no
verification tests
...
* [ ] Error handling (malformed input, missing fields, invalid trust
levels)
* [ ] Accessibility (N/A)
...
* *HIGH* — A2ML is a critical format in the ecosystem. 4 AffineScript
source files + 3 Idris2 ABI + 2 Zig FFI files with ZERO functional
tests. The fuzz directory contains only a placeholder.txt. As a library
consumed by other projects, this needs comprehensive tests.
